...
export default class Field {
  ...
  createView(parent) {
    ...
    const leftHintsTable = createHTMLElement('table', 'row-hints-table', leftHintsBigCell);

    const mainFieldWrapper = createHTMLElement('td', 'main-field-wrapper', rowSecond);
    this.mainField = createHTMLElement('table', 'main-field', mainFieldWrapper);
    ...
    for (let j = 0; j < this.topHints[0].length; j += 1) {
      ...
      for (let i = 0; i < this.topHints.length; i += 1) {
        const text = this.topHints[i][j] ? this.topHints[i][j] : 0;
        const rowTopHint = createHTMLElement('td', 'row-top-hint', rowTopHints, text);
      }
    }
